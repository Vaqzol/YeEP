...
// ข้อมูลเส้นทาง (รวมแดงและน้ำเงิน)
const busRoutes = [
    {
        id: 'red', color: '#FF4500',
        stops: [
        ...
        rawPath: [
            ...
            [102.014412, 14.895023], [102.013721, 14.895412], [102.012875, 14.895701], [102.012421, 14.896629], [102.011710, 14.897994],
...
function startTracking() {
    ...
    const busId = busInput.value;
    if (!busId) { alert("กรุณากรอกเลขรถ"); return; }

    const statusFooter = document.getElementById("status");
    statusFooter.innerText = `🔄 กำลังค้นหา BUS ${busId}...`;

    if (trackingInterval) clearInterval(trackingInterval);
    fetchGps(busId);
    trackingInterval = setInterval(() => fetchGps(busId), 3000);
}

function fetchGps(busId) {
    fetch(`/gps?bus=${busId}`)
        .then(res => res.json())
        .then(data => {
            ...
            if (lat === 0 && lng === 0) {
                statusFooter.innerText = `⚠️ ไม่พบสัญญาณ BUS ${busId}`;
                return;
            }

            // กำหนดสีรถตาม ID
            let busColor = '#22c55e'; // สีเขียว (Default)
            if (['1','2','3'].includes(String(busId))) busColor = '#FF4500'; // แดง
            else if (['4','5','6'].includes(String(busId))) busColor = '#2563EB'; // น้ำเงิน
            ...
            map.panTo([lat, lng]); // เลื่อนแผนที่ตามรถ
            statusFooter.innerText = `📍 BUS ${busId} อยู่ที่: ${lat.toFixed(5)}, ${lng.toFixed(5)}`;
        });
}

function drawAllRoutes() {
    busRoutes.forEach(route => {
        // วาดเส้นทาง (แปลง Lng,Lat -> Lat,Lng)
